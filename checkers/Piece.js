...
class Normal_piece extends Piece {
    ...
    mustAttack(board) {
        for (var i = -2; i < 3; i += 4) {
            for (var j = -2; j < 3; j += 4) {
                if (this.canAttack(this.matrixPosition.x + i, this.matrixPosition.y + j, board) != null) {
                    return true;
                }
            }
        }
    ...
    canAttack(x, y, board) {
        ...
        if (!board.getPieceAt(x, y) && abs(x - this.matrixPosition.x) == abs(y - this.matrixPosition.y) && abs(y - this.matrixPosition.y) == 2) {
            var x_mid = this.matrixPosition.x + (x - this.matrixPosition.x) / 2;
            var y_mid = this.matrixPosition.y + (y - this.matrixPosition.y) / 2;
            var attackingPiece = board.getPieceAt(x_mid, y_mid);
            if (attackingPiece != null && !this.attackingAllies(x_mid, y_mid, board)) {
    ...
    possibleMoves(board) {
        ...
        for (var i = -1; i < 2; i += 2) {
            for (var j = -1; j < 2; j += 2) {
                ...
                var x_next = this.matrixPosition.x + 2 * i;
                var y_next = this.matrixPosition.y + 2 * j;
                
                var attackingPiece = board.getPieceAt(x, y);
                if (!this.attackingAllies(x, y, board) && attackingPiece != null && this.withinBounds(x_next, y_next) && !board.isPieceAt(x_next, y_next)) {
                    moves.push(createVector(x_next, y_next));
                    attacked = true;
                }
            }
        }
        ...
        for (var i = -1; i < 2; i += 2) {
            ...
            if (!attacked && !board.isPieceAt(x, y) && this.withinBounds(x, y)) {
                if (board.isPieceAt)
